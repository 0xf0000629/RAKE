...
- ``<имя переменной> <конечное значение> < +/- > loop <команды> loop end`` - исполнить команды внутри цикла и выполняет операцию инкремента/декремента, пока выбранная переменная не дойдёт до конечного значения (нач. - включительно, кон. - не включительно).
...
\
Стек данных - в него кладутся данные при выполнении операций.\
